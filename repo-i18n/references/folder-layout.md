...
| `assets/bundles/{code}.json` | App UI strings (loaded at runtime by Flutter). One per locale. |
...
│   │   └── zh.json                  # App UI translations (中文)
│   ├── docs/
│   │   ├── en.json                  # README content (English)
│   │   └── zh.json                  # README content (中文)
│   └── templates/
...
- `lib/l10n/app_localizations.dart` — `supportedLocales` must list every locale.
...
- Loads `assets/bundles/{code}.json`; keys starting with `_` are ignored
  ...
  (`zh`): `{...base, ...variant}` — missing keys fall back to base, then to `en`.
- `allKeys` static set must contain every UI key used.
